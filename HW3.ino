...
void loop() {
  ...
  //Player 1
  if(digitalRead(Button1_Pin) == HIGH && btn1_Pressed == false && gameState==true){
    btn1_Pressed = true;
    pos += 5; //角度加五度
    if(pos>=180||pos<=0){
      gameState=false;
  ...
  //Player 2
  if(digitalRead(Button2_Pin) == HIGH && btn2_Pressed == false && gameState==true){
    btn2_Pressed = true;
    pos -= 5; //角度減五度
    if(pos<=0||pos>=180){
      gameState=false;
  ...
  if(digitalRead(Button1_Pin) == HIGH && btn1_Pressed == false && digitalRead(Button2_Pin) == HIGH && btn2_Pressed == false){
    pos=90;
    gameState=true;
    digitalWrite(27, LOW);
    }
